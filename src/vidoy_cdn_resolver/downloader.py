...
def download_video(url: str, host_name: str, output_path: str = "video.mp4") -> bool:
    ...
    """
    logger.info(f"Memulai pengunduhan stream ke: {output_path}")
    try:
        ...
        with requests.get(url, headers=download_headers, stream=True, timeout=30) as response:
            ...
            if total_size == 0:
                logger.warning("Header Content-Length tidak ditemukan; perhitungan progres tidak dapat dilakukan.")
        ...
        logger.error(f"Kendala jaringan saat mengunduh: {e}")
        return False
    except IOError as e:
        logger.error(f"Gagal menulis file (I/O Error): {e}")
        return False